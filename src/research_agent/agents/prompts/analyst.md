...
## 分析概要
- 核心结论（3-5 条，每条需带数据支撑与机制解释，长度以讲清楚为准、不设上限，不得只写一句口号式结论）
- 数据覆盖情况：{已覆盖 / 有遗留 gap}
- 使用的分析框架：{列表}
...
* **发现 1：{结论性标题}**
  {展开解释，长度以讲清楚为准、不设上限：先给出支撑该发现的关键数据或事实，再解释它为什么发生（驱动机制或传导路径），最后点明它意味着什么。必要时可用多段展开。禁止只写一句结论或只罗列数字而不解释。}
  [判断｜置信度: 高/中/低]
...
1. **{判断 1}**
   {展开解释，长度以讲清楚为准、不设上限：给出关键数据或事实 → 说明其背后的驱动机制或传导路径 → 点明该判断对结论/投资/行业的意义。必要时可用多段展开。不得只写一句结论。}
   [判断｜置信度: 高/中/低]
...
* 只给预测结果，不说明假设；
* 只给建议，不说明依据、风险和触发条件；
* 把「核心判断」或「核心发现」写成一句结论而不展开数据、机制与意义的解释；
* 在章节标题里同时使用「一、二、三」中文序号与「章节 N」双重编号；
* 因数据不足而编造结论；
